  ...
  useEffect(() => {
    const fetchCredits = async () => {
      try {
        ...
        setCredits(data?.data?.credit || 0); // adjust key based on API
      } catch (error) {
  ...
  return (
    <aside className="w-64 border-r border-slate-200 flex flex-col bg-white z-10 h-screen sticky top-0">
      ...
      <nav className="flex-1 px-4 space-y-1 mt-4">
        ...
        <Link href="/dashboard/text-voice" className={getLinkClass("/dashboard/text-voice")}>
          <Mic2 size={18} />
          Text to Voice
        </Link>

        <Link href="/dashboard/voice-clone" className={getLinkClass("/dashboard/voice-clone")}>
          <Copy size={18} />
          Voice Clone
        </Link>

        <Link href="/dashboard/library" className={getLinkClass("/dashboard/library")}>
          <Library size={18} />
          Library
        </Link>
        ...
        <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 flex flex-col gap-3">
          ...
          <div className="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden">
            <div
              ...
              // style={{ width: `${Math.min((credits / 3000) * 100, 100)}%` }} // adjust max credits
            ></div>
          </div>

          <Link
            href="/pricing"
            className="w-full py-2 bg-primary text-white text-xs font-bold rounded-lg hover:opacity-90 transition-opacity flex justify-center items-center"
          >
            Upgrade Plan
          </Link>
        </div>
